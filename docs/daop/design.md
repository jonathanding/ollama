...
| **Dedicated `measureMulMat()` for quantized benchmarks** | GGML `mul_mat` requires weight=quantized dtype, activation=f32/f16 (mixed dtype). Generic `measureOp()` creates all tensors with same dtype. Only MUL_MAT has this constraint — FLASH_ATTN uses uniform f16, element-wise ops use f32. |
